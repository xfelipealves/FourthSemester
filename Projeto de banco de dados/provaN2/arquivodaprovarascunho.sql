...
--rascunho
with
cleitin as ( --retorna a qntde de cada assento vendido
    select v.numero_voo,
           v.codigo_aeronave,
           a.capacidade_passageiros,
           p.numero_assento,
    ...
    from voo v
             inner join aeronave a on v.codigo_aeronave = a.codigo_aeronave
             inner join passageiro p on v.numero_voo = p.numero_voo
    group by v.numero_voo, a.codigo_aeronave, p.numero_assento
    --having count(*) >=2
    order by v.numero_voo, a.codigo_aeronave, p.numero_assento
),
cleitin2 as ( --retorna a soma de passagens vendidas
    select c.numero_voo,
           c.capacidade_passageiros,
           sum(c.qtde) as qtde_passagens_vendidas
    from cleitin c
    group by c.numero_voo, c.codigo_aeronave, c.capacidade_passageiros
    order by c.numero_voo
)
select
    c2.numero_voo,
    c2.capacidade_passageiros,
    c2.qtde_passagens_vendidas
from cleitin c
inner join cleitin2 c2 on c.numero_voo = c2.numero_voo
where c.qtde >=2 --restringe apenas para aqueles voos que deram overbooking
group by c2.numero_voo, c2.capacidade_passageiros, c2.qtde_passagens_vendidas
order by c2.numero_voo;
