...
//void sort::RadixSort(int *V, int N)
//{
//    Radix_Sort(V,N,10);
//}
...
//{ //MODELO USUAL
//    //k é a o range de elementos possiveis
//    int* vet=new int[k + 1];
//    for (int i = 0; i <= k; i++)  //O(k)
//    {//zerando o vetor criado
//        vet[i] = 0;
//    }
...
{//MODELO ADAPTADO
    //k é a o range de elementos possiveis
    int* vet=new int[10];       //adaptado
    for (int i = 0; i < 10; i++)  //O(k)    //adaptado
    {//zerando o vetor criado
        vet[i] = 0;
    }
    for (int i = 0; i < N; i++)    //O(n)
    {
        vet[(A[i]/k)%10]++;        //O(n)   //adaptação..
    }
    for (int i = 1; i < 10; i++)   //O(k)   //adaptado
    {
        vet[i] += vet[i - 1];   //O(k)
        /* acumula a quantdade de elementos alocados nas posições
         * anteriores + a a da posição atual */
    ...
    for (int i = N - 1; i >= 0; i--) //O(n)
    {
        aux[vet[ (A[i]/k)%10 ] -1] = A[i];//O(n)    //adaptação...
        vet[ (A[i]/k)%10 ]--;        //O(n)     //adaptação....
    }

    for (int i = 0; i < N; i++)   //O(n)   //adaptado
    {
        A[i] = aux[i];      //O(n)      //adaptado
    }
}

void sort::Radix_Sort(int *A, int N) //O(dN)
{//MODELO ADAPTADO
    int M=getMax(A,N);
...
//void sort::Radix_Sort(int *A, int N, int d) //O(dN)
//{ //Modelo USUAL
//    for(int i=1, x=10;i<=d;i++, x=x*10) //O(d)
//    {
//        //ordenar todos os elementos de A apenas pelo dígito i, com um algoritmo
//        //estavel   //d * O(n)
//    }
//}

int sort::getMax(int *A, int N)
{
    int M=A[0];
    for (int i=1; i<N; i++)
    {
