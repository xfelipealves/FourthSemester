...
//    cout<<partida<<" "<<s3<<endl;
    ...
    int d[locais.size()];
    int p[locais.size()];
    int index[locais.size()];
    aeroportos.Dijkstra(s3,d,p,index);
    ...
    stack<int> ordem_output, indexoutput;
    int conti=0;
    ordem_output.push(s2);
//    indexoutput.push(index[s2]);
//    cout<<";"<<ordem_output.top();
    //cout<<"."<<indexoutput.top();
    for (int i=s2; i!=s3; i=p[i], conti++)
    {
        ordem_output.push(p[i]);
        indexoutput.push(index[i]);
//        cout<<";"<<ordem_output.top();
//        cout<<"."<<indexoutput.top();
    }
//    cout<<endl;
    ...
    {
        index_inverso[i]=indexoutput.top();
        indexoutput.pop();
    }

    /*Validacao dos valores obtidos*/
//    for (int i=0;i<conti;i++)
//    {
//        cout<<pai_inverso[i]<<" ";
//    }
...
//    cout<<endl;
    ...
    for (int i=0;i<conti-1;i++)
    {
        list<voosBrabos>::iterator qq=
                aeroportos.M[pai_inverso[i]][pai_inverso[i+1]].voozin.begin();
        //a aresta é indicada por indexoutput[i]!

        int z=0;
        for(;qq!=aeroportos.M[pai_inverso[i]][pai_inverso[i+1]].voozin.end() and z!=index_inverso[i]; qq++, z++);


        if (i>0 and qq->voo==vooAnterior)//compara com o anterior
        {
            //cout<<endl;
        }
        else
